...
READY_FUNCTIONS: list[FunctionInfo] = [
    FunctionInfo(
        ...
        prepared_function_details=PreparedFunctionDetails(type=WorkerType.TEST, identifier=""),
    )
]


def create_execution(worker_status: WorkerStatus, outcome: Optional[ExecutionOutcome]) -> ExecutionSummary:
...
def create_invocation(
    ...
    for _ in range(num_successful_terminated_executions):
        executions.append(create_execution(WorkerStatus.TERMINATED, ExecutionOutcome.SUCCEEDED))

    for _ in range(num_aborted_terminated_executions):
        executions.append(create_execution(WorkerStatus.TERMINATED, ExecutionOutcome.ABORTED))

    for _ in range(num_failed_terminated_executions):
        executions.append(create_execution(WorkerStatus.TERMINATED, ExecutionOutcome.FAILED))
...
def assert_results(
    ...
    actual_ids_to_terminate = {invocation.invocation_id for invocation in classification.invocations_to_terminate}
    actual_ids_to_create_executions_for = {
        invocation.invocation_id for invocation in classification.invocations_to_create_executions_for
    }
    ...
    if expected_ids_to_create_executions_for is None:
        ...
        assert actual_ids_to_create_executions_for == expected_ids_to_create_executions_for
...
def test_classify_with_an_existing_running_execution() -> None:
    invocation_id = "inv-1"
    running_invocations = [create_invocation(invocation_id, FUNCTION_NAME, num_running_executions=1)]

    classification = classify_running_invocations(running_invocations, READY_FUNCTIONS, TIME)
...
def test_classify_with_an_existing_pending_execution() -> None:
    invocation_id = "inv-1"
    running_invocations = [create_invocation(invocation_id, FUNCTION_NAME, num_pending_executions=1)]
...
def test_classify_with_cancellation_request() -> None:
    invocation_id = "inv-1"
    running_invocations = [create_invocation(invocation_id, FUNCTION_NAME, cancellation_requested=True)]
...
def test_classify_with_no_existing_executions() -> None:
    ...
    classification = classify_running_invocations(running_invocations, READY_FUNCTIONS, TIME)
    assert_results(classification, expected_ids_to_create_executions_for={invocation_id})


def test_classify_with_execution_failing_but_not_reaching_retries_limit() -> None:
    invocation_id = "inv-1"
    running_invocations = [create_invocation(invocation_id, FUNCTION_NAME, num_failed_terminated_executions=1)]

    classification = classify_running_invocations(running_invocations, READY_FUNCTIONS, TIME)
    assert_results(classification, expected_ids_to_create_executions_for={invocation_id})


def test_classify_with_execution_suffering_hardware_failure_but_not_reaching_retries_limit() -> None:
    invocation_id = "inv-1"
    running_invocations = [create_invocation(invocation_id, FUNCTION_NAME, num_terminated_executions_without_outcome=1)]
...
def test_classify_with_function_capacity_enough_to_create_execution_for_some_but_not_all_invocations() -> None:
    invocation_id_1 = "inv-1"  # running an execution
    invocation_id_2 = "inv-2"  # not yet running an execution
    invocation_id_3 = "inv-3"  # not yet running an execution
    running_invocations = [
    ...
    id_of_invocation_to_create_execution_for = classification.invocations_to_create_executions_for[0].invocation_id
