...
class HttpRequestHandler:
    ...
    def post(self, path: str, request_body: BaseModel, response_body_type: type[T]) -> T:
        ...
        if 200 <= response.status_code <= 299:
            ...
            error_response_body = self._extract_object_from_response(response, HttpStandardErrorResponse)
            raise HttpHandlingError(f"Status code: {response.status_code}, Detail: {error_response_body.detail}")
    ...
    def _extract_object_from_response(response: Response, object_type: type[T]) -> T:
        try:
            ...
            raise HttpHandlingError(f"Status code: {response.status_code}, Cannot parse response as JSON: {str(ex)}")

        try:
            return object_type.model_validate(response_dict)
        except ValidationError as ex:
            raise HttpHandlingError(f"Status code: {response.status_code}, Cannot deserialise response: {str(ex)}")
