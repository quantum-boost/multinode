...
def import_multinode_object_from_file(filepath: Path) -> Multinode:
    module = _import_python_module_from_file(filepath)
    multinode_objects = _extract_multinode_objects_from_module(module)

    if len(multinode_objects) == 0:
        raise ImportError(f"Could not find a Multinode object in {filepath}.")
    if len(multinode_objects) > 1:
        raise ImportError(
            f"Found more than one Multinode object in {filepath}. "
